...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		width = saveWidth(points);
		height = saveHeight(points);
		makeSkirt(points);
	}
	
	private int saveWidth(TPoint[] points) {
		int curr = -1;
	...
	private int saveHeight(TPoint[] points) {
		int count = -1;
		for(int i=0; i<points.length; i++) {
			if(points[i].y > count) count = points[i].y;
		}
		return count+1;
	}
	...
	private void makeSkirt(TPoint[] points) {
		skirt = new int[width];
		int curr = 10000;
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] tp = new TPoint[body.length];
		for(int i=0; i<body.length; i++) {
			TPoint curr = new TPoint(height-body[i].y-1, body[i].x);
			tp[i] = curr;
		}
		Piece p = new Piece(tp);
		return p;
	}
	...
	public boolean equals(Object obj) {
		...
		for(int i=0; i<body.length; i++) {
			for(int j=0; j<body.length; j++) {
				if(other.getBody()[i].equals(body[j])) {
					count++;
				}
			}
		}
		if(count == body.length) return true;
		return false;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr = root;
		for(int i=0; i<3; i++) {
			Piece tmp = curr.computeNextRotation();
			curr.next =  tmp;
			curr = curr.next;
		}
		curr.next = root;
